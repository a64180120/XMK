  ...
  <div class="payList">
      ...
      <el-row :gutter="10" style="margin-bottom: 20px">
        <el-col :span="24">
          ...
          <div class="top-btn">
            ...
            <el-button class="btn" size="mini" @click="print">打印</el-button>
          </div>
        </el-col>
      </el-row>
      <el-row :gutter="20">
                ...
                <li :class="{'my-10':detail.Mst.FApproval == 2 ||detail.Mst.FApproval == 0}">
                  ...
                  >
                    <el-tooltip class="payTooltip" :disabled="!account">
                      ...
                      <span
                        slot="content"
                      >{{(accountList.length && account&&account!='0')?(accountList.find(item=>{return item.PhId == account})).FBankname:''}}</span>
                    </el-tooltip>
                  </div>
                  <div v-else>
                ...
                <li>
                  ...
                  >
                    <el-tooltip class="payTooltip" :disabled="!detail.Mst.FPaymethod">
                      ...
                      <span
                        slot="content"
                      >{{detail.Mst.FPaymethod.length==15&&FPaymethodList.length>0?(FPaymethodList.find(item=>item.PhId == detail.Mst.FPaymethod)).TypeName:''}}</span>
                    </el-tooltip>
                  </div>
                  <div v-else>
                    <el-tooltip class="payTooltip">
                      ...
                      >
                        <template slot-scope="scope">
                          ...
                          <span v-else-if="scope.column.property=='FResultmsg'">
                            <el-tooltip
                              v-if="scope.row.FResultmsg&&scope.row.FState!=1"
    ...
    <div class="content printcontent" ref="printArea">
      ...
      <table style="display:none">
        ...
        <tr>
          <td colspan="3" class="secondTitle">审批流程</td>
        </tr>
        <tr>
        ...
        <tr v-if="auditMsg.length" v-for="(item,index) in auditMsg">
      ...
      <table>
        ...
        <tr>
          <td colspan="6" class="secondTitle">申报拨付明细</td>
        </tr>
        <tr class="tbTitle">
          <td>项目编码</td>
          <td>项目名称</td>
        ...
        <template v-if="detail.Dtls.length">
          <tr v-for="(item,index) in detail.Dtls">
            <td
              ...
              :rowspan="(index<detail.Dtls.length-1)&&(detail.Dtls[index+1].XmProjcode==item.XmProjcode)&&[...detail.Dtls].splice(index+1).findIndex(i=>i.XmProjcode==item.XmProjcode)+2"
            >{{item.XmProjcode}}</td>
            <td
              class="left"
              v-if="index==0||detail.Dtls[index-1].XmProjcode!=item.XmProjcode"
              :rowspan="(index<detail.Dtls.length-1)&&(detail.Dtls[index+1].XmProjcode==item.XmProjcode)&&[...detail.Dtls].splice(index).findIndex(i=>i.XmProjcode==item.XmProjcode)+2"
            >{{item.XmProjname}}</td>
