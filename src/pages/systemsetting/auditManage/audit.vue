<template>
  <div class="auditLiuCheng">
    <topHandle :title="'系统管理在线工作平台'"
               @refresh="refresh">
      <div class="btnCon">
        <div v-if="menuButton.approvalflow_add=='True'"
        ...
        <div v-if="menuButton.approvalflow_edit=='True'"
        ...
        <div v-if="menuButton.approvalflow_delete=='True'"
             @click.stop="deleteAudit"
             class="handle">
    ...
    <div class="container">
      <div class="searchCon">
        <search placeholder="流程编码/流程名称"
                v-model="serachVal"
                @btnClick="search"></search>
      </div>
      <div class="content">
        <div class="auditType">
          <div class="formArea">
            ...
            <div class="tableBody">
              <table>
                ...
                <thead>
                  <tr :class="{trActive:type.checked}"
                      v-for="(type,n) in typeList"
                      :key="n">
                    <td @click.stop="typechoose(type)"
                        style="padding: 0 5px;cursor:pointer;">
                      <el-checkbox v-model="type.checked"
                                   @click.stop.native="1"
                                   @change="typechoose(type,'change')">{{n+1}}</el-checkbox>
                    </td>
                    <td>
          ...
          <div class="typeHandle">
            <div>
              ...
              <div v-if="menuButton.approvalflow_typeedit=='True'"
                   @click.stop="showAuditTypeAdd('update')"
                   class="typeUpdate">
                <img src="@/assets/images/update.png"
                     alt=""> <span>修改</span>
              </div>
              <div v-if="menuButton.approvalflow_typedelete=='True'"
                   @click.stop="deletetype"
                   class="typeDelete">
          ...
          <div class="formArea">
            <div class="tableHead ">
              <table>
                ...
                <thead>
                  <tr>
                    <td @click.stop="allChecked('change')"
            ...
            <div class="tableBody">
              <table>
                ...
                <thead>
                  <tr :class="{trActive:audit.checked}"
                      v-for="(audit,m) in auditList"
                      :key="m">
                    <td @click.stop="choose(audit)"
                        style="cursor:pointer;padding: 0 5px;">
                      <el-checkbox @click.stop.native="1"
                                   @change="choose(audit,'change')"
                                   v-model="audit.checked">{{m+1}}</el-checkbox>
                    </td>
                    <td>
    ...
    <fDialog :title="(auditBtn=='add'?'新增':'修改')+'审批流'"
             :visible.sync="auditAddShow">
      <audit-add v-if="auditAddShow"
                 :orglist="orglist"
                 :splx="splx"
                 :auditinfo="auditinfo"
                 :type="auditBtn"
    ...
    <fDialog :title="'审批类型'+(typeBtn=='add'?'新增':'修改')"
             :visible.sync="auditTypeAddShow">
      <audittype-add v-if="auditTypeAddShow"
                     :type="typeBtn"
                     :typeinfo="typeinfo"
                     @add-cancle="typeCancle"></audittype-add>
    </fDialog>
  </div>
</template>
