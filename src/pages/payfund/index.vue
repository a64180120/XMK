            ...
            <table>
              ...
              <thead>
                <tr>
                  ...
                  <td title="申报单金额（元）">申报单金额（元）</td>
                  <td title="申报日期">申报日期</td>
                  <td title="审批状态">审批状态</td>
                  <td title="支付单编号">支付单编号</td>
                  <td title="支付状态">支付状态</td>
                  <td title="申报说明">申报说明</td>
                </tr>
              </thead>
            </table>
          </div>
...
export default {
  ...
  methods: {
    ...
    getChartList: function (val) {
      ...
      this.getAxios('GBK/PaymentMstApi/GetAmountOfMoney', param)
        .then(res => {
          this.chartData.chart = [
            { name: '可申报', value: res.Sum },
            { name: '冻结', value: res.Frozen },
            { name: '已使用', value: res.Use }
          ]
          let maxNum = 0
    ...
    getAllProByBm: function () {
      ...
      this.getAxios('GYS/BudgetMstApi/GetBudgetMstList', param)
        .then(res => {
