<template>
  ...
  <section class="prerojectnewproject">
        ...
        <div class="btn-left">
          <div class="bottom-info">
            <ul>
              ...
              <li>
                <span>申报日期：{{(new Date()).getFullYear()+'-'+((new Date()).getMonth()<10?'0'+((new Date()).getMonth()+1):(new
                                Date()).getMonth())+'-'+((new
                                Date()).getDate()<10?'0'+((new
                                Date()).getDate()):(new
                                Date()).getDate())}}</span>
                                </li>
                                <li>
          ...
          <div v-show="tabindex == 3"
            ...
            <div class="line2"
              ...
              <el-popover placement="bottom"
                ...
                <div slot="reference"
                     style="width: 300px;border-bottom: 1px solid #e3e3e3">
            ...
            <div class="line3"
              ...
              <div class="tableBody">
                <table style="border-right: none;border-top: none;border-bottom: none"
                  ...
                  <tbody>
                    <tr v-if="target.targetTableData.length !==0"
                        v-for="(item,idx) in target.targetTableData">
                      <td>{{idx}}</td>
                      <td v-if="idx==0"
                          :rowspan="target.targetTableData.length">
                        <add-br :value="target.targetType.FName"></add-br>
                      </td>
                      <td v-for="(itemType) in tarType"
                          :rowspan="itemType.sum"
                          v-if="idx==itemType.beginIdx">{{item.FTargetClassCode_EXName}}</td>
                      <td>
                        <el-input class="td-ipt"
                                  v-model="item.FTargetName"></el-input>
                      </td>
                      <td>
                        <el-input class="td-ipt"
                                  v-model="item.FTargetValue"></el-input>
                      </td>
                      <td>
                        <el-input class="td-ipt"
                                  @change="targetWeight()"
                                  v-model="item.FTargetWeight"></el-input>
                      </td>
                      <td @mouseenter="mouseEnter(idx)">
                        <el-input class="td-ipt"
                                  v-model="item.FTargetDescribe"></el-input>
                      </td>
                      <td class="enable">
                        <div v-show="nowIndex == idx"
                             class="icon active">
                          <div>
                    ...
                    <tr v-if="target.targetTableData.length ==0">
                      <td colspan="7"
                          style="border-bottom: 1px solid #ccc">暂无数据</td>
    ...
    <el-dialog modal-append-to-body
               ...
               class="setBuyDialog">
      <set-buy v-if="setBuyDialog.openDialog"
               :data="setBuyDialog.data"
               :fund="budgetDetail.FSourceOfFundsGroup"></set-buy>
    </el-dialog>
    <el-dialog modal-append-to-body
               append-to-body
    ...
    <el-dialog :visible.sync="uploadVis"
               ...
               title="附件上传"
               :close-on-click-modal="false">
      <file-up @submit="submitFn"
               v-if="uploadVis"
               :fileItem="choosedIndexAndPro.pro.QtAttachments"></file-up>
    </el-dialog>
  </section>
</template>
