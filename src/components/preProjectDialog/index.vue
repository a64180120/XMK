<template>
  ...
  <section class="prerojectnewproject">
      ...
      <div class="left-box">
        ...
        <ul>
          <li>
            <span>项目名称：</span>
            <div
              @click="openTextarea(projSurvey,'FProjName')"
              class="opentextarea"
              style="width:100%;text-align:left;line-height:32px;padding-left:15px;"
            >
              <el-tooltip v-if="projSurvey.FProjName" :content="projSurvey.FProjName">
          ...
          <li>
            <span>申报部门：</span>
            <el-select v-model="projSurvey.FDeclarationDept" size="small" placeholder="必选">
              <el-option
                v-for="(item,idx) in projGroup.FDeclarationDeptGroup"
                :key="idx"
                :label="item.DeftName"
                :value="item.DeftCode"
              ></el-option>
          ...
          <li>
            <span>预算部门：</span>
            <el-select v-model="projSurvey.FBudgetDept" size="small" placeholder="必选">
              <el-option
                v-for="(item,idx) in projGroup.FBudgetDeptGroup"
                :key="idx"
                :label="item.oname"
                :value="item.ocode"
              ></el-option>
        ...
        <div class="right-box-container">
            ...
            <div class="list listBodyCon">
              <div class="listBody">
                ...
                <ul v-else v-for="(item,index) in budgetdetailData" :key="index">
                  <li>{{index+1}}</li>
                  <li>
                    <el-input v-model="item.FName" @change="valueChange(item)"  placeholder="必填" />
                  </li>
                  <li>
                    <el-input
                  ...
                  <li>
                    <el-select v-model="item.FPaymentMethod" size="small" placeholder="必选">
                      <el-option
                  ...
                  <li>
                    <el-radio v-model="item.FIfPurchase" :label="1">是</el-radio>
                    <el-radio v-model="item.FIfPurchase" :label="0">否</el-radio>
                    <i
                      @click="item.FIfPurchase?setBuy(item):''"
                      :class="{'el-icon-setting':true,'canSetting':item.FIfPurchase}"
                    ></i>
                  </li>
                  <li class="enable" @click="openTextarea(item,'FOtherInstructions')">
                     <el-input v-model="item.FOtherInstructions" placeholder="必填" />

                      <el-tooltip v-if="item.FOtherInstructions" :content="item.FOtherInstructions">
                        <span style="display: inline-block;max-width: 100%;">{{item.FOtherInstructions}}</span>
                      </el-tooltip>
                    <div class="icon active">
                      <div>
                        <img @click.stop="addBudgetdetail(item)" src="@/assets/images/jia.png" alt />
                      </div>
                      <div>
                        <img @click.stop="delBudgetdetail(item,index)" src="@/assets/images/jian.png" alt />
                      </div>
                    </div>
                  </li>
                </ul>
              ...
              <div class="listBottom-right">
                ...
                <ul>
                  <li v-for="(item,idx) in budgetDetail.SourceOfFundsGroup">
                    ...
                    <span
                      class="money"
                    >{{budgetdetailData.filter(i=>i.SourceOfFunds===item.value).reduce((prev,cur)=>prev+parseFloat((Number((cur.FAmount).replace(/[,]/g, ''))).toFixed(2)),0) | NumFormat}}</span>
                  </li>
                </ul>
              </div>
            </div>
          ...
          <div v-show="tabindex == 2" class="budgetdetail">
            ...
            <div class="list plan listBodyCon">
              <div class="listBody">
                <div style="cursor:pointer" v-if="!ImplPlanPanelData.length">请添加明细</div>
                <ul v-else v-for="(item,index) in ImplPlanPanelData" :key="index" :class="[delRow == index?'select-row':'']">
                  <li>{{index+1}}</li>
                  <li>
                    <el-input v-model="item.FName" placeholder="必填" />
                  </li>
                  <li class="enable">
                    <el-date-picker
                      size="small"
          ...
          <div v-show="tabindex == 3" class="jxtarget">
            ...
            <div class="line3" v-show="projSurvey.xjEvaluate =='1'">
              <div class="tableHead">
                <table>
                  <colgroup>
                    <col width="10%" />
    ...
    <el-dialog
      ...
      class="setBuyDialog"
    >
      <textarea-dialog v-if="textareaDialogData.openDialog" :maxWord="maxWord" :data="textareaDialogData"></textarea-dialog>
    </el-dialog>
  </section>
</template>
