...
export default {
  ...
  methods: {
    ...
    getAppvalProc() {
      ...
      this.getAxios('/GAppvalPost/GetAppvalProcAndOperator', data)
        .then(res => {
          if (res && res.Status === 'success') {
            this.$set(this.approvalFollow, 0, res.Process)
            this.nextApprovaler = res.AppvalPost.Operators
            if (res.AppvalPost.Operators.length == 0) {
              this.isLast = true //已经到最后一个岗位
            } else {
              this.isLast = false
            }
    ...
    submit() {
      ...
      if (
        this.isAgree === '9' &&
        this.nextApprovaler.length !== 0 &&
        this.operatorID.length === 0
      ) {
        this.$msgBox.error({
          content: '下一审批人不能为空'
        })
      ...
      this.formAxios('GSP/GAppvalRecord/PostApprovalRecordList', formData)
        .then(res => {
    ...
    getBackApprovalPost() {
      ...
      this.getAxios('/GAppvalPost/GetBackApprovalPost', data)
        .then(res => {
          if (res && res.Status == 'success') {
            console.log(res)
