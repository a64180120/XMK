  ...
  <section>
    <div class="dialog-content">
      <el-row :gutter="10"
        ...
        <el-col :span="24">
          <div class="top-btn">
            ...
            <el-button :disabled="PaymentXmDtl.length==0"
                       class="btn"
                       size="mini"
                       @click="save(0)">保存</el-button>
            <el-button v-if="menu['fund_check']"
                       :disabled="PaymentXmDtl.length==0"
                       class="btn"
                       size="mini"
                       @click="save(1)"
                       style="padding: 0">保存并送审</el-button>
            <el-button class="btn"
                       size="mini"
                       @click="add">增加项目</el-button>
            <el-button class="btn"
                       size="mini"
                       @click="delPro">删除项目</el-button>
          </div>
        </el-col>
      </el-row>
      <el-row :gutter="20"
          ...
          <el-card class="payText"
            ...
            <div style="margin: 0 14px">
              ...
              <table class="proTab">
                ...
                <tr>
                  <td class="right"
                      style="padding-right: 20px">申报单名称:</td>
                  <td class="left"
                      colspan="3"
                      style="border-bottom: 1px solid #9acefb;">
                    <el-input placeholder="30字以内"
                              maxlength="30"
                              show-word-limit
                              v-model="PaymentMst.FName"></el-input>
                  </td>
                </tr>
                <tr>
                  <td class="right"
                      style="padding-right: 20px">申&nbsp报&nbsp部&nbsp门:</td>
                  <td class="left"
                      style="border-bottom: 1px solid #9acefb">
                    <el-tooltip :content="PaymentMst.FOrgname&&PaymentMst.FDepname?PaymentMst.FOrgname+'-'+PaymentMst.FDepname:''"
                  ...
                  <td class="right"
                      style="border-bottom: 1px solid #fff;min-width: 170px">金&nbsp额&nbsp合&nbsp计（元）:</td>
                  <td class="left"
                      style="border-bottom:1px solid #9acefb;overflow: hidden">
                    <el-tooltip :content="PaymentMst.FAmountTotal | NumFormat"
                                popper-class="tooltipCla"
                                placement="bottom-start">
                ...
                <tr>
                  <td class="right"
                      style="padding-right: 20px">申&nbsp报&nbsp说&nbsp明:</td>
                  <td colspan="3"
                      style="border-bottom: 1px solid #9acefb">
                    <el-tooltip :disabled="!PaymentMst.FDescribe||(PaymentMst.FDescribe&&PaymentMst.FDescribe.length<60)"
                                :content="PaymentMst.FDescribe"
                                popper-class="tooltipCla"
                                placement="bottom-start">
                      <p>
                        <el-input placeholder="100字以内"
                                  resize="none"
                                  maxlength="100"
                                  show-word-limit
                                  v-model="PaymentMst.FDescribe"></el-input>
                      </p>
                    </el-tooltip>
                  </td>
                </tr>
      ...
      <el-row class="content"
        ...
        <el-col :span="24"
          ...
          <el-card class="payText"
                   v-for="(item,pindex) in PaymentXmDtl">
            <div slot="header"
              ...
              <span style="float: right">
                ...
                <span style="margin-left: 20px;cursor: pointer"
                      @click="delPro(pindex)"><i class="el-icon-close"></i></span>
              </span>
            ...
            <div>
              <div style="margin: 0 20px;">
                ...
                <span>
                  <el-select size="small"
                             v-model="item.PaymentXm.XmProjcode"
                             @change="changePro(pindex)">
                    <el-option v-for="(pro,index) in prodata.Mst"
                               v-if="index>0 "
                               :label="pro.FProjName"
                               :key="pro.FProjCode"
                               :value="pro.FProjCode"
                               :disabled="pro.checked"></el-option>
                  </el-select>
                </span>
                <span style="float: right">
                  <span>
              ...
              <div style="height: 40px;line-height: 40px;background-color: #d7d7d7;padding:0 10px;margin-top: 10px;">
                ...
                <span style="float: right;margin-right: 10px">
                  <el-checkbox v-model="lineCopy[pindex]">复制行</el-checkbox>
                </span>
              </div>
            </div>
                ...
                <tbody>
                  <tr v-for="(mx,index) in item.PaymentDtls">
                    ...
                    <td class="right"
                      ...
                      <span class="num"
                            v-if="!mx.checked"
                            @click="changeInp(pindex,index)">{{mx.FAmount | NumFormat}}</span>
                      <input :id="pindex+'-'+index"
                             v-if="mx.checked"
                             :autofocus="mx.checked"
                    ...
                    <td>
                      <el-tooltip :disabled="mx.FRemarks&&mx.FRemarks.length<15"
                                  :content="mx.FRemarks"
                                  popper-class="tooltipCla"
                                  placement="bottom-start">
                        <p style="width: 300px;">{{mx.FRemarks}}</p>
                        <p><input v-model="mx.FRemarks "
                                 @focus="topClick(pindex,index)"
                                 maxlength="100" /></p>
                      </el-tooltip>
                    ...
                    <td class="iconTd">
                      <i class="el-icon-plus"
                         @click="addDtl(pindex,index)"></i>
                      <i class="el-icon-minus"
                         @click="delDtl(pindex,index)"></i>
                    </td>
                  </tr>
                  <tr>
                    <td></td>
    ...
    <el-dialog title="提示"
      ...
      <span slot="footer"
        ...
        <button class="cancelBtn"
                @click="msgType=false">取消</button>
        <button class="confirmBtn"
                style="margin-left: 30px"
                @click="deleteApply">确定</button>
      </span>
    </el-dialog>
    <!--消息提示窗-->
    <el-dialog title="提示"
               ...
               append-to-body
               :close-on-click-modal="false">
      <p>{{msg}} &nbsp;（{{time}}s）后自动关闭</p>
      <span slot="footer">
        <button class="confirmBtn"
                @click="handleClose">确定</button>
      </span>
    </el-dialog>
    <!--组织树弹窗-->
    <!--    <el-dialog id="orgdialog" width="350px" title="组织树"
               :visible.sync="orgType" :append-to-body="true">-->
    <orgtree :visible.sync="orgType"
             :data="orgList"
             :checkedOrg="checkedOrgList"
             @confirm="confirmOrg"></orgtree>
    ...
    <el-dialog width="600px"
      ...
      <span slot="footer"
            style="text-align: center">
        <button class="cancelBtn"
                @click="orgDetailType=false">取消</button>
        <button class="confirmBtn"
                style="margin-left: 30px"
                @click="confirmProDetail(false)">保存</button>
      </span>
    </el-dialog>
    <!--送审-->
