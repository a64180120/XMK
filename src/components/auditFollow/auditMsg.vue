  ...
  <div class="auditMsg">
    <ul :class="[info.FApproval=='9'?'oxford-gray':(info.FApproval=='0'?'light-gray':(info.FApproval=='1'?'blue':'red'))]">
      <template v-if="!isApproval">
        <li>
          <i v-if="index !=nowNum"
             :class="[info.FApproval=='9'?'logo-sptg':(info.FApproval=='0'?'logo-wsp':(info.FApproval=='1'?'logo-now':'logo-wtg'))]"></i>
          <i v-else
             class="logo-spz "></i>
        ...
        <li :class="[index==nowNum?'nowClass':'']"
            v-if="info.JudgeRefer !== 1 && (info.FApproval ===9 ||info.FApproval ===2) ">
          意见：
          <span>{{info.FApproval ===2?'不同意':'同意'}}</span>
        </li>
        <li :class="[index==nowNum?'nowClass':'']"
            v-if="info.JudgeRefer !== 2">备注：{{info.FOpinion}}</li>
        <li :class="[index==nowNum?'nowClass':'']"
            v-if="info.JudgeRefer !== 2">
          附件:
          <span @click.stop="showAttech"
                v-if="info.QtAttachments ===null"
                class="attenchment">无</span>
          <span v-else
                v-for="(item,idx) in info.QtAttachments">
            <span @click.stop="showAttech(info.QtAttachments)"
                  class="attenchment"
                  style="word-break: break-word">{{item.BName}}</span>
